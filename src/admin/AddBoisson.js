...
import { createBoisson, getCategories } from './apiAdmin';
...
const AddBoisson = () => {
 ...
 setValues({
    ...
    const clickSubmit = event => {
        ...
        createBoisson(user._id, token, formData).then(data => {
            if (data.error) {
    ...
    const newPostForm = () => (
        <form className="mb-3" onSubmit={clickSubmit}>
            ...
            <button className="btn btn-outline-primary">Ajouter à la Carte Boisson</button></center>
        </form>
    );
    ...
    const showSuccess = () => (
        <div className="alert alert-info" style={{ display: createdBoisson ? '' : 'none' }}>
            <h2>{`${createdBoisson}`} est ajouté avec succèes!</h2>
        </div>
    );
